...
INFO=$( cat $(ls ~/go/src/logs/out/0_*) | grep 'ID:0' | cut -d" " -f4- | cut -d"|" -f2- )

TEMP=$( cd ~/go/src/logs/client && cat $(ls) | grep 'Operation Latency:' | cut -d" " -f4 )
OP="${TEMP// /$'\n'}"
AVG_OP=$( echo "$OP" | awk -vx=0 '{x += $1} END {print x/NR}' )

TEMP=$( cd ~/go/src/logs/out && cat $(ls) | grep 'Message Complexity:' | cut -d" " -f3 )
COM="${TEMP// /$'\n'}"
AVG_COM=$( echo "$COM" | awk -vx=0 '{x += $1} END {print x/NR}' )

TEMP=$( cd ~/go/src/logs/out && cat $(ls) | grep 'Message Size:' | cut -d" " -f3 )
SIZE="${TEMP// /$'\n'}"
